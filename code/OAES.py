...
class AES:
    ...
    def shift_rows(self,state):
        ...
        '''
         #normal shiftrows
        state[1][0], state[1][1], state[1][3], state[1][3] = state[1][1], state[1][3], state[1][3], state[1][0]
        state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[3][2], state[2][0], state[2][1]
    ...
    def mix_column(self,col):
        ...
        col[0] ^= all_xor ^ self.xtime(col[0] ^ col[1])
        col[1] ^= all_xor ^ self.xtime(col[1] ^ col[2])
        col[2] ^= all_xor ^ self.xtime(col[2] ^ col[3])
        col[3] ^= all_xor ^ self.xtime(c_0 ^ col[3])
    ...
    def encrypt(self,data: bytes) -> bytes:
        ...
        for round in range(1, self.nr):
            ...
            #self.add_round_key(state, self.key_schedule, round)#modified part
    ...
    def inv_mix_column(self,col):
        ...
        col[0] = self.xtimes_0e(c_0) ^ self.xtimes_0b(c_1) ^ self.xtimes_0d(c_2) ^ self.xtimes_09(c_3)
        col[1] = self.xtimes_09(c_0) ^ self.xtimes_0e(c_1) ^ self.xtimes_0b(c_2) ^ self.xtimes_0d(c_3)
        col[2] = self.xtimes_0d(c_0) ^ self.xtimes_09(c_1) ^ self.xtimes_0e(c_2) ^ self.xtimes_0b(c_3)
        col[3] = self.xtimes_0b(c_0) ^ self.xtimes_0d(c_1) ^ self.xtimes_09(c_2) ^ self.xtimes_0e(c_3)
    ...
    def inv_mix_column_optimized(self,col):
        u = self.xtime(self.xtime(col[0] ^ col[2]))
        v = self.xtime(self.xtime(col[1] ^ col[3]))
        col[0] ^= u
        col[1] ^= v
        col[2] ^= u
        col[3] ^= v
    ...
    def inv_mix_columns_optimized(self,state) :
        for r in state:
            self.inv_mix_column_optimized(r)
        self.mix_columns(state)
